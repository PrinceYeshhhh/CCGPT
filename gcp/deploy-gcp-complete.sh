...
# CustomerCareGPT Complete Google Cloud Run Deployment Script
# This script provides a complete deployment solution for Google Cloud Run
...
echo -e "${BLUE}🚀 CustomerCareGPT Complete Google Cloud Run Deployment${NC}"
echo -e "${BLUE}=====================================================${NC}"
...
create_service_account() {
    ...
    gcloud iam service-accounts create "$SERVICE_ACCOUNT" \
        --display-name="CustomerCareGPT Service Account" \
        --description="Service account for CustomerCareGPT Cloud Run services" || true
...
create_database() {
    ...
    gcloud sql instances create customercaregpt-db \
        ...
        --backup \
        --enable-ip-alias \
        --root-password=YourSecurePassword123! || true
...
deploy_services() {
    ...
    # Deploy ChromaDB
    echo -e "${YELLOW}🚀 Deploying ChromaDB service...${NC}"
    gcloud run deploy customercaregpt-chromadb \
        ...
        --max-instances 2 \
        --min-instances 0 \
        --set-env-vars "CHROMA_SERVER_HOST=0.0.0.0,CHROMA_SERVER_HTTP_PORT=8001"
...
show_urls() {
    ...
    CHROMADB_URL=$(gcloud run services describe customercaregpt-chromadb --platform managed --region "$REGION" --format 'value(status.url)')
    ...
    echo "- gcp/cloud-run-configs.yaml"
    echo "- gcp/secrets-config.yaml"
    echo "- gcp/cloud-sql-config.yaml"
    echo "- gcp/redis-config.yaml"
}
...
cleanup() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        gcloud run services delete customercaregpt-chromadb --platform managed --region "$REGION" --quiet || true
        ...
        gcloud iam service-accounts delete "$SERVICE_ACCOUNT@$PROJECT_ID.iam.gserviceaccount.com" --quiet || true
...
show_help() {
    echo -e "${BLUE}CustomerCareGPT Complete Google Cloud Run Deployment Script${NC}"
    echo ""
