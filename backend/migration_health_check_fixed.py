#!/usr/bin/env python3
"""
Fixed migration health check script.
            ...
            with engine.connect() as connection:
                ...
                if 'users' in tables:
                    if "postgresql" in self.database_url:
                        ...
                        user_columns = [row[1] for row in result.fetchall()]  # SQLite uses column 1 for name
    ...
    def check_migration_consistency(self):
        ...
        try:
            ...
            try:
                ...
                self.warnings.append(f"Could not check migration consistency: {e}")
    ...
    def check_migration_scripts(self):
        ...
        for script in scripts_to_check:
            ...
            if not os.path.exists(script_path):
                self.issues.append(f"Migration script not found: {script}.py")
            else:
                # Check if script is executable
                try:
                    result = subprocess.run(
                        ["python", "-c", f"import {script}"],
                    ...
                    if result.returncode == 0:
                        self.successes.append(f"Migration script {script}.py is valid")
                    else:
                        self.warnings.append(f"Migration script {script}.py has issues: {result.stderr}")
                except Exception as e:
                    self.warnings.append(f"Could not validate migration script {script}.py: {e}")
            ...
            with engine.connect() as connection:
                ...
                try:
                    ...
                    try:
                        ...
                        self.issues.append(f"Database permission issues: {e2}")
                        return False
